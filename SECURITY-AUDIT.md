...
| NAT-1 | 🔴 | **DM E2E implemented** (Phase 3) — direct messages now X25519 ECDH (from Ed25519 identities) → HKDF-SHA256 (salt=conversationId) → XChaCha20-Poly1305 (conversationId as AAD). Shared module mirrored in native (`utils/e2e-crypto.ts`) + PWA (`lib/e2e-crypto.ts`); interop proven by `scratch/e2e-vector.mjs` (8/8). Legacy plaintext-v1 still readable; node stores opaque ciphertext (no server change). **Limits:** group/system threads still plaintext-v1; no forward secrecy (static keys); metadata still visible to node. tsc clean both apps; **runtime native↔native + native↔PWA round-trip pending** (needs rebuild+redeploy) |
| NAT-2 | 🔴 | Code fixed (untracked from git) — ⚠️ **manual: rotate the Firebase/FCM key** |
| PWA-4 | 🔴 | **Fixed** — The identity import now properly writes to the IndexedDB store using `importIdentity()`, and device wipe properly removes it via `wipeIdentity()`. |
