...
export function MarketplacePage({ identity, marketClickCount = 0, openPostId, onPostOpened, onNavigate }: Props) {
    ...
    // ⚡ Bolt: Memoize post filtering to prevent unnecessary re-renders of the entire list
    const filteredPosts = useMemo(() => {
        let result = posts.filter(p => p.status === 'active' && (!identity || p.authorPublicKey !== identity.publicKey));
        ...
        return result;
    }, [posts, identity?.publicKey, searchQuery, radiusSettings]);
