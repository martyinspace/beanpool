...
export function PostAuthorTrust({ callsign, energyCycled = 0, rating, mode = 'full', className = '', avatarUrl, publicKey, onOpenProfile }: PostAuthorTrustProps) {
    ...
    if (mode === 'compact') {
        return (
            <>
                <div 
                    className={`flex items-center gap-1 ${publicKey ? 'cursor-pointer hover:opacity-80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-nature-500 rounded' : ''} ${className}`}
                    onClick={handleClick}
                    onKeyDown={publicKey ? handleKeyDown : undefined}
    ...
    return (
        <>
            <div 
                className={`flex items-center gap-1.5 ${publicKey ? 'cursor-pointer hover:opacity-80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-nature-500 rounded' : ''} ${className}`}
                onClick={handleClick}
                onKeyDown={publicKey ? handleKeyDown : undefined}
                role={publicKey ? "button" : undefined}
                tabIndex={publicKey ? 0 : undefined}
