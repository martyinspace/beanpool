        ...
        >
            ...
            {hasPhoto ? (
                <div className={`relative w-full rounded-[20px] overflow-hidden shadow-sm ${isGrid ? 'h-[110px] mb-3' : 'h-[180px] mb-4'}`}>
                    ...
                    {/* Recurring Overlay */}
                    {post.repeatable && (
                        <div className={`absolute left-2 bg-terra-500/90 backdrop-blur-md text-white font-bold tracking-tight shadow-md ${isGrid ? 'bottom-2 px-2 py-1 rounded-lg text-[10px]' : 'bottom-3 px-3 py-1.5 rounded-xl text-xs'}`}>
                            ↻ RECURRING
                        </div>
                    )}
                </div>
                ...
                <div className={`relative w-full rounded-[20px] bg-oat-50 dark:bg-nature-900 flex items-center justify-center shadow-inner ${isGrid ? 'h-[80px] mb-3' : 'h-[100px] mb-4'}`}>
                    ...
                    {/* Recurring Overlay */}
                    {post.repeatable && (
                        <div className={`absolute left-2 bg-terra-500/90 backdrop-blur-md text-white font-bold tracking-tight shadow-md ${isGrid ? 'bottom-2 px-2 py-1 rounded-lg text-[10px]' : 'bottom-3 left-3 px-3 py-1.5 rounded-xl text-xs'}`}>
                            ↻ RECURRING
                        </div>
                    )}
                </div>
