...
export function getConversationsByMember(pubkey: string): Conversation[] {
    ...
    if (conversationIds.length > 0) {
        ...
        for (let i = 0; i < conversationIds.length; i += chunkSize) {
            ...
            const chunkParts = db.prepare(partsQuery).all(...chunk) as any[];
    ...
    if (allPeerPubkeys.size > 0) {
        ...
        const chunkSize = 900;
        for (let i = 0; i < pubkeysArray.length; i += chunkSize) {
            const chunk = pubkeysArray.slice(i, i + chunkSize);
            const placeholders = chunk.map(() => '?').join(',');
            const membersQuery = `SELECT public_key, callsign, avatar_url FROM members WHERE public_key IN (${placeholders})`;
            const chunkMembers = db.prepare(membersQuery).all(...chunk) as any[];
            allMembers.push(...chunkMembers);
        }
    ...
    if (postIds.length > 0) {
        ...
        for (let i = 0; i < postIds.length; i += chunkSize) {
            ...
            const chunkPosts = db.prepare(postsQuery).all(...chunk) as any[];
