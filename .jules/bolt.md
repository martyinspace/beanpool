...
## 2026-05-23 - [O(N) Allocations in Array Filtering on Relational Data]
**Learning:** In various HTTP and federation endpoints, fetching the entire member registry via `getMembers()` and then using `.find()` repeatedly to locate specific members causes significant O(N) array mapping and database retrieval per request. This blocks the single-threaded Node.js event loop on large member bases.
**Action:** Replaced `getMembers().find()` with direct O(1) SQLite primary-key index lookup using the `getMember(publicKey)` function.
