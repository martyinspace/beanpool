...
## 2026-05-20 - [CRITICAL] Fix authorization bypass on critical endpoints
**Vulnerability:** Several critical API endpoints (e.g., `/api/ratings`, `/api/reports`, `/api/friends/`, `/api/recovery/`, `/api/push-tokens`, `/api/members/preferences`) were not included in the `isProtected` list inside the `requireSignature` middleware. This allowed an attacker to bypass cryptographic signature verification and impersonate other users simply by setting the `X-Public-Key` header or the target pubkey in the request payload.
**Learning:** Middleware configurations using explicit inclusion lists (`isProtected`) are prone to omissions. Newly added or refactored API routes can easily bypass security checks if the inclusion list is not updated simultaneously. This architecture puts the burden on developers to remember to protect endpoints.
**Prevention:** Consider refactoring the middleware to an exclusion list (e.g., allow specific public endpoints and protect everything else by default), or apply the middleware specifically to the router instances/groups that require authentication, rather than using a global conditional check based on path prefixes.
